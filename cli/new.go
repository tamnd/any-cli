...
site-specific commands.
  ...
  any new lobsters -d ~/code --binary lob`,
	...
	f.StringVar(&opt.Host, "host", "", "site host the client and URI driver target (default <site>.com)")
...
func printNextSteps(out interface{ Write([]byte) (int, error) }, site scaffold.Site, destDir string, remote bool) {
	...
	fmt.Fprintf(&b, "Next:\n")
	fmt.Fprintf(&b, "  cd %s\n", destDir)
	fmt.Fprintf(&b, "  make build && ./bin/%s page about\n", site.Binary)
	fmt.Fprintf(&b, "  # model your records in %s/%s.go, then declare ops in %s/domain.go\n", site.LibPkg, site.LibPkg, site.LibPkg)
	fmt.Fprintf(&b, "  # the same domain.go is the binary and the %s:// driver a host like ant blank-imports\n", site.Scheme)
	if !remote {
